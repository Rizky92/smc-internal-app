...
use App\Livewire\Pages\Keuangan\TarifRalan;
use Livewire\Livewire;
use Tests\TestCase;

/**
 * Seam B: the tarif rawat jalan list.
 *
 * A pure read-only report - the only write path is the paired
 * ImportTarifRalan modal, tested separately. This suite is a smoke test:
 * the query mounts and runs without error, and the "Import" trigger is
 * gated on the same permission the modal itself checks.
 */
class TarifRalanTest extends TestCase
{
    /**
     * @test
     */
    ...
    {
        $petugas = $this->petugasWithPermissions(['keuangan.tarif-ralan.read'], '99999901');

        Livewire::actingAs($petugas)
            ->test(TarifRalan::class)
            ->call('loadProperties')
    ...
    {
        $tanpaIzin = $this->petugasWithPermissions(['keuangan.tarif-ralan.read'], '99999901');
        $denganIzin = $this->petugasWithPermissions(['keuangan.tarif-ralan.read', 'keuangan.tarif-ralan.create'], '99999902');
