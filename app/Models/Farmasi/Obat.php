...
class Obat extends Model
{
    ...
    public function scopePemakaianObatNAPZA(Builder $query, string $tglAwal = '', string $tglAkhir = '', string $golongan = 'narkotika'): Builder
    {
        ...
        $sqlSelect = <<<'SQL'
            ...
            gudang.kd_bangsal,
            gudang.nm_bangsal,
            ifnull((select riwayat_barang_medis.stok_awal from riwayat_barang_medis where riwayat_barang_medis.kode_brng = databarang.kode_brng and riwayat_barang_medis.kd_bangsal = gudang.kd_bangsal and riwayat_barang_medis.tanggal between ? and ? order by riwayat_barang_medis.tanggal asc, riwayat_barang_medis.jam asc limit 1), (select riwayat_barang_medis.stok_akhir from riwayat_barang_medis where riwayat_barang_medis.kode_brng = databarang.kode_brng and riwayat_barang_medis.kd_bangsal = gudang.kd_bangsal and riwayat_barang_medis.tanggal < ? order by riwayat_barang_medis.tanggal desc, riwayat_barang_medis.jam desc limit 1)) as stok_awal,
            (select sum(mutasibarang.jml) from mutasibarang where mutasibarang.kode_brng = databarang.kode_brng and mutasibarang.kd_bangsalke = gudang.kd_bangsal and date(mutasibarang.tanggal) between ? and ?) tf_masuk,
            (select sum(detailpesan.jumlah2) from detailpesan join pemesanan on detailpesan.no_faktur = pemesanan.no_faktur where detailpesan.kode_brng = databarang.kode_brng and pemesanan.kd_bangsal = gudang.kd_bangsal and pemesanan.tgl_pesan between ? and ?) penerimaan_obat,
            (select sum(riwayat_barang_medis.masuk) from riwayat_barang_medis where riwayat_barang_medis.kode_brng = databarang.kode_brng and riwayat_barang_medis.kd_bangsal = gudang.kd_bangsal and riwayat_barang_medis.posisi = 'Piutang' and riwayat_barang_medis.tanggal between ? and ?) piutang_masuk,
            (select sum(detailhibah_obat_bhp.jumlah2) from detailhibah_obat_bhp join hibah_obat_bhp on detailhibah_obat_bhp.no_hibah = hibah_obat_bhp.no_hibah where detailhibah_obat_bhp.kode_brng = databarang.kode_brng and hibah_obat_bhp.kd_bangsal = gudang.kd_bangsal and hibah_obat_bhp.tgl_hibah between ? and ?) hibah_obat,
            (select sum(detreturjual.jml_retur) from detreturjual join returjual on detreturjual.no_retur_jual = returjual.no_retur_jual where detreturjual.kode_brng = databarang.kode_brng and returjual.kd_bangsal = gudang.kd_bangsal and returjual.tgl_retur between ? and ?) retur_pasien,
            (select sum(riwayat_barang_medis.masuk) from riwayat_barang_medis where riwayat_barang_medis.posisi = 'Pemberian Obat' and riwayat_barang_medis.status = 'hapus' and riwayat_barang_medis.kode_brng = databarang.kode_brng and riwayat_barang_medis.kd_bangsal = gudang.kd_bangsal and tanggal between ? and ?) hapus_beriobat,
            (select sum(detail_pemberian_obat.jml) from detail_pemberian_obat where detail_pemberian_obat.kode_brng = databarang.kode_brng and detail_pemberian_obat.kd_bangsal = gudang.kd_bangsal and tgl_perawatan between ? and ?) pemberian_obat,
            (select sum(detailjual.jumlah) from detailjual join penjualan on detailjual.nota_jual = penjualan.nota_jual where detailjual.kode_brng = databarang.kode_brng and penjualan.kd_bangsal = gudang.kd_bangsal and penjualan.tgl_jual between ? and ?) penjualan_obat,
            (select sum(riwayat_barang_medis.keluar) from riwayat_barang_medis where riwayat_barang_medis.kode_brng = databarang.kode_brng and riwayat_barang_medis.kd_bangsal = gudang.kd_bangsal and riwayat_barang_medis.posisi = 'Piutang' and riwayat_barang_medis.tanggal between ? and ?) piutang_keluar,
            (select sum(mutasibarang.jml) from mutasibarang where mutasibarang.kode_brng = databarang.kode_brng and mutasibarang.kd_bangsaldari = gudang.kd_bangsal and date(mutasibarang.tanggal) between ? and ?) tf_keluar,
            (select sum(detreturbeli.jml_retur2) from detreturbeli join returbeli on detreturbeli.no_retur_beli = returbeli.no_retur_beli where detreturbeli.kode_brng = databarang.kode_brng and returbeli.kd_bangsal = gudang.kd_bangsal and returbeli.tgl_retur between ? and ?) retur_supplier
            SQL;
    ...
    public function scopePemakaianObatNAPZAAgregat(Builder $query, string $tglAwal = '', string $tglAkhir = '', string $golongan = 'narkotika'): Builder
    {
        ...
        return $query
            ...
            ->selectRaw('SUM(detail.stok_awal) as stok_awal')
            ->selectRaw('SUM(detail.tf_masuk) as tf_masuk')
            ->selectRaw('SUM(detail.penerimaan_obat) as penerimaan_obat')
            ->selectRaw('SUM(detail.piutang_masuk) as piutang_masuk')
            ->selectRaw('SUM(detail.hibah_obat) as hibah_obat')
            ->selectRaw('SUM(detail.retur_pasien) as retur_pasien')
            ->selectRaw('SUM(detail.hapus_beriobat) as hapus_beriobat')
            ->selectRaw('SUM(detail.pemberian_obat) as pemberian_obat')
            ->selectRaw('SUM(detail.penjualan_obat) as penjualan_obat')
            ->selectRaw('SUM(detail.piutang_keluar) as piutang_keluar')
            ->selectRaw('SUM(detail.tf_keluar) as tf_keluar')
            ->selectRaw('SUM(detail.retur_supplier) as retur_supplier')
            ->groupBy([
                'detail.kode_brng',
                'detail.nama_brng',
                'detail.kode_golongan',
