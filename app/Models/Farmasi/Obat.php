...
class Obat extends Model
{
    ...
    public function scopeDaruratStok(Builder $query, string $tglAwal = '', string $tglAkhir = ''): Builder
    {
        $tglAwal = carbon($tglAwal)->subWeeks(2)->toDateString();
        $tglAkhir = carbon($tglAkhir)->now()->toDateString();
        ...
        $sqlSelect = <<<'SQL'
            ...
            (ifnull((select round(sum(detail_pengeluaran_obat_bhp.jumlah), 2) from detail_pengeluaran_obat_bhp join pengeluaran_obat_bhp on detail_pengeluaran_obat_bhp.no_keluar = pengeluaran_obat_bhp.no_keluar where detail_pengeluaran_obat_bhp.kode_brng = databarang.kode_brng and pengeluaran_obat_bhp.tanggal between ? and ?), 0)) stok_keluar_medis_14_hari,
            (ifnull((select round(sum(detail_pemberian_obat.jml), 2) from detail_pemberian_obat where detail_pemberian_obat.kode_brng = databarang.kode_brng and detail_pemberian_obat.tgl_perawatan between ? and ?), 0) + ifnull((select round(sum(detailjual.jumlah), 2) from detailjual join penjualan on detailjual.nota_jual = penjualan.nota_jual where detailjual.kode_brng = databarang.kode_brng and penjualan.tgl_jual between ? and ?), 0)) ke_pasien_14_hari,
            (ifnull((select round(sum(dp.jumlah), 2) from piutang p join detailpiutang dp on p.nota_piutang = dp.nota_piutang where dp.kode_brng = databarang.kode_brng and p.tgl_piutang between ? and ?), 0)) piutang_14_hari,
        ...
        $this->addSortColumns([
            ...
            'stok_keluar_medis_14_hari' => DB::raw('(ifnull((select round(sum(detail_pengeluaran_obat_bhp.jumlah), 2) from detail_pengeluaran_obat_bhp join pengeluaran_obat_bhp on detail_pengeluaran_obat_bhp.no_keluar = pengeluaran_obat_bhp.no_keluar where detail_pengeluaran_obat_bhp.kode_brng = databarang.kode_brng and pengeluaran_obat_bhp.tanggal between ? and ?), 0))'),
            'ke_pasien_14_hari'         => DB::raw('(ifnull((select round(sum(detail_pemberian_obat.jml), 2) from detail_pemberian_obat where detail_pemberian_obat.kode_brng = databarang.kode_brng and detail_pemberian_obat.tgl_perawatan between ? and ?), 0) + ifnull((select round(sum(detailjual.jumlah), 2) from detailjual join penjualan on detailjual.nota_jual = penjualan.nota_jual where detailjual.kode_brng = databarang.kode_brng and penjualan.tgl_jual between ? and ?), 0))'),
            'piutang_14_hari'           => DB::raw('(ifnull((select round(sum(dp.jumlah), 2) from piutang p join detailpiutang dp on p.nota_piutang = dp.nota_piutang where dp.kode_brng = databarang.kode_brng and p.tgl_piutang between ? and ?), 0))'),
