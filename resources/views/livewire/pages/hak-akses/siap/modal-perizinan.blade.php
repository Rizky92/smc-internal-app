<div>
    ...
    <x-modal
        id="modal-perizinan"
        title="Set perizinan untuk {{ $roleName }}"
        livewire
    >
        <x-slot name="body" class="p-0" style="overflow-x: hidden">
            <form
                id="form-perizinan"
                wire:submit.prevent="{{ $roleId !== -1 ? 'update' : 'create' }}"
            >
                <x-row-col class="sticky-top bg-white pt-3 pb-2 px-3">
                    <x-flash />
                    <div
                        class="{{ Arr::toCssClasses(['form-group', 'mt-3' => session()->has(['flash.type', 'flash.message'])]) }}"
                    >
                    ...
                    <ul class="form-group">
                        @foreach ($this->permissions as $group => $items)
                            @foreach ($items as $key => $name)
                                <li
                                    class="{{ Arr::toCssClasses(['custom-control custom-checkbox', 'mt-3' => $loop->first && ! $loop->parent->first]) }}"
                                >
                                    <input
                                        type="checkbox"
