        ...
        <x-slot name="header">
            ...
            <x-row-col-flex class="mt-2">
                ...
                <x-filter.select
                    ...
                    :options="['semua' => 'Semua', 'ralan' => 'Rawat Jalan', 'ranap' => 'Rawat Inap']"
                    selected="semua"
                />
                '
            ...
            <x-row-col-flex
                :class="Arr::toCssClasses(['mt-2', 'pb-3' => user()->can('keuangan.account-receivable.validasi-piutang')])"
            >
            ...
            @can('keuangan.account-receivable.validasi-piutang')
                ...
                <x-row-col-flex class="mt-2">
                    <x-filter.label constant-width class="font-weight-bold">
                        Dibayar:
                    </x-filter.label>
        ...
        <x-slot name="body">
            ...
            >
                ...
                <x-slot name="body">
                    @forelse ($this->dataAccountReceivable as $item)
                        ...
                        <x-table.tr>
                            ...
                            @can('keuangan.account-receivable.validasi-piutang')
                                <x-table.td>
                                    <div
                                        class="form-group m-0 position-relative"
                                    >
                                        <input
                                            type="number"
                    ...
                    @empty
                        <x-table.tr-empty
                            :colspan="user()->can('keuangan.account-receivable.validasi-piutang') ? 22 : 20"
                            padding
                        />
                    @endforelse
                </x-slot>
                <x-slot name="footer">
                    <x-table.tr>
                        <x-table.th
                            :colspan="user()->can('keuangan.account-receivable.validasi-piutang') ? 3 : 2"
                        />
                        <x-table.th title="TOTAL :" />
                        <x-table.th
                            :title="rp(optional($this->dataTotalAccountReceivable)['totalPiutang'])"
                        />
                        <x-table.th
                            :title="rp(optional($this->dataTotalAccountReceivable)['totalCicilan'])"
                        />
                        <x-table.th
                            :title="rp(optional($this->dataTotalAccountReceivable)['totalSisaCicilan'])"
                        />
                        @if (user()->can('keuangan.account-receivable.validasi-piutang'))
                            <x-table.th
                                :title="rp(optional($this->dataTotalAccountReceivable)['totalDiskonPiutang'])"
                            />
                        @endif

                        <x-table.th colspan="6" />
                        <x-table.th
                            :title="rp(optional(optional($this->dataTotalAccountReceivable)['totalSisaPerPeriode'])->get('periode_0_30'))"
                        />
                        <x-table.th
                            :title="rp(optional(optional($this->dataTotalAccountReceivable)['totalSisaPerPeriode'])->get('periode_31_60'))"
                        />
                        <x-table.th
                            :title="rp(optional(optional($this->dataTotalAccountReceivable)['totalSisaPerPeriode'])->get('periode_61_90'))"
                        />
                        <x-table.th
                            :title="rp(optional(optional($this->dataTotalAccountReceivable)['totalSisaPerPeriode'])->get('periode_90_up'))"
                        />
                    </x-table.tr>
                </x-slot>
            </x-table>
