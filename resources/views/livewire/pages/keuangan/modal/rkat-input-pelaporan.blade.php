<div>
    ...
    @endpush
    <x-modal id="modal-input-pelaporan-rkat" :title="($this->isUpdating() ? 'Edit Data Penggunaan RKAT' : 'Input Data Penggunaan RKAT') .
        ' Tahun ' .
        $this->tahun" livewire centered>
            ...
            <x-form id="form-input-pelaporan-rkat" livewire :submit="$this->isUpdating() ? 'update' : 'create'">
                <x-row-col class="sticky-top bg-white">
                    <div class="form-group">
                        ...
                        <x-form.select2 id="anggaran-bidang-id" model="anggaranBidangId" :options="$this->dataRKATPerBidang" placeholder="-"
                            width="full-width" />
                    ...
                    <div class="form-group mt-3">
                        ...
                        <input type="text" id="keterangan" wire:model.defer="keterangan"
                    ...
                    <div class="form-group mt-3">
                        <div class="d-flex justify-content-start align-items-center">
                            <span class="d-block font-weight-bold" style="width: calc(75% - 1.6rem)">Nama
                                Pengeluaran</span>
                        ...
                        <ul class="p-0 m-0 mt-2 mb-3 d-flex flex-column" style="row-gap: 0.5rem" id="detail-pemakaian">
                            @foreach ($this->detail as $index => $item)
                                <li class="d-flex justify-content-start align-items-center m-0 p-0"
                                    wire:key="detail-pelaporan-{{ $index }}">
                                    <input type="text" class="form-control form-control-sm"
                                        wire:model.defer="detail.{{ $index }}.keterangan">
                                    <span class="ml-4 text-sm" style="width: 3rem">Rp.</span>
                                    <input type="text" class="form-control form-control-sm text-right w-25"
                                        wire:model.defer="detail.{{ $index }}.nominal">
                        ...
                        @can('keuangan.rkat-pelaporan.update')
                            <x-button size="sm" variant="secondary" title="Tambah Detail" icon="fas fa-plus"
        ...
        <x-slot name="footer" class="justify-content-start">
            @can('keuangan.rkat-pelaporan.update')
                ...
                <x-button size="sm" variant="primary" type="submit" class="ml-2" id="simpandata" title="Simpan"
                    icon="fas fa-save" form="form-input-pelaporan-rkat" />
            @endcan
        </x-slot>
    </x-modal>
</div>
