    ...
    @push('js')
        <script>
            ...
            $(document).on('data-saved', () => {
                $('#modal-import-tarif-ranap').modal('hide');
            });
        </script>
    @endpush

    <x-modal id="modal-import-tarif-ranap" title="Import Tarif Ranap" livewire centered>
        <x-slot name="body">
            <x-flash class="mx-3 mt-3" />
            <x-form id="form-import-tarif-ranap" livewire wire:submit.prevent="importData">
                <x-form.import-file id="upload-tarif-ranap" model="fileImport" accept=".xlsx, .xls" template="templates/template-import-tarif-ranap.xlsx" />
            </x-form>
        </x-slot>
