<div>
    ...
    <x-modal id="modal-input-posting-jurnal" size="xl" :title="'Posting Jurnal Baru'" livewire centered static>
                    ...
                    <x-table zebra hover sticky nowrap>
                                ...
                                <x-table.tr>
                                    <x-table.td>
                                        <div style="width: 100%" wire:ignore>
                                            <select id="kd_rek_{{ $index }}" wire:model.defer="detail.{{ $index }}.kd_rek" class="form-control form-control-sm select2 input-sm" data-index="{{ $index }}">
                        ...
                        <x-slot name="footer">
                            <x-table.tr>
                                ...
                                <x-table.td>{{ rp($this->totalDebet) }}</x-table.td>
                                <x-table.td>{{ rp($this->totalKredit) }}</x-table.td>
                            </x-table.tr>
                        </x-slot>
                    </x-table>
                </x-row-col>
                        ...
                        <x-slot name="body">
                            @forelse ($this->jurnalSementara as $index => $jurnal)
                                ...
                                <x-table.tr style="background-color: rgb({{ $odd }})">
                                    ...
                                    <x-table.td rowspan="{{ $count }}">{{ $jurnal['tgl_jurnal'] }}</x-table.td>
                                    <x-table.td rowspan="{{ $count }}">{{ $jurnal['jam_jurnal'] }}</x-table.td>
                                    <x-table.td rowspan="{{ $count }}">{{ $jurnal['jenis'] === 'U' ? 'UMUM' : 'PENYESUAIAN' }}</x-table.td>
                                    <x-table.td rowspan="{{ $count }}">{{ $jurnal['keterangan'] }}</x-table.td>
                                    <x-table.td>{{ $firstDetail['kd_rek'] }}</x-table.td>
                                    <x-table.td>{{ $this->rekening->get($firstDetail['kd_rek']) }}</x-table.td>
                                    <x-table.td>{{ rp($firstDetail['debet']) }}</x-table.td>
                                    <x-table.td>{{ rp($firstDetail['kredit']) }}</x-table.td>
                                </x-table.tr>
                                @foreach (collect($jurnal['detail'])->skip(1) ?? [] as $detail)
                                    <x-table.tr style="background-color: rgb({{ $odd }})">
                                        <x-table.td class="border-0">{{ $detail['kd_rek'] }}</x-table.td>
                                        <x-table.td class="border-0">{{ $this->rekening->get($detail['kd_rek']) }}</x-table.td>
                                        <x-table.td class="border-0">{{ rp($detail['debet']) }}</x-table.td>
                                        <x-table.td class="border-0">{{ rp($detail['kredit']) }}</x-table.td>
                                    </x-table.tr>
                                @endforeach
                            @empty
        ...
        <x-slot name="footer" class="justify-content-start">
            ...
            <span class="text-sm font-weight-bold" wire:loading wire:target="create">Tunggu...</span>
            <x-button size="sm" variant="primary" type="submit" class="ml-2" id="simpandata" title="Simpan" icon="fas fa-save" form="form-input-posting-jurnal" wire:target="create" wire:loading.class="d-none" wire:loading.class.remove="btn" :disabled="empty($this->jurnalSementara)" />
        </x-slot>
    </x-modal>
</div>
