            ...
            <div class="timeline">
                @forelse ($this->aktivitasUser as $date => $timeline)
                    <div class="time-label">
                        <span
                            class="bg-white text-dark text-sm px-3 border font-weight-bold"
                        >
                    ...
                    @foreach ($timeline as $item)
                        <div
                            class="{{ Arr::toCssClasses(['mt-3' => $loop->first && ! $loop->parent->first]) }}"
                        >
                            <i class="fas fa-angle-right bg-dark"></i>
                            <div
                                class="timeline-item border-0 shadow-none bg-transparent"
                                style="margin-top: -0.2rem"
                            >
                                <div
                                    class="timeline-body mt-n1 d-flex justify-content-start align-items-start"
                                >
                                    <span
                                        class="badge badge-secondary text-xs"
                                        style="margin-inline-end: 1.25rem"
                                    >
                                    ...
                                    <div class="mt-n1 flex-fill">
                                        ...
                                        >
                                            Mengunjungi
                                            <a
                                                href="{{ Route::has($item->route_name) ? route($item->route_name) : '#' }}"
                                            >
                                                {{ str($item->breadcrumbs)->afterLast('/')->trim() }}
                                            </a>
                                        </h6>
                                        <div
                                            class="d-flex w-100 justify-content-start align-items-baseline"
                                        >
                                            <p class="m-0 d-block">
                                                {{ $item->breadcrumbs }}
                                            </p>
                                            <div
                                                class="bg-light border rounded px-2 ml-auto"
                                                style="
                                            ...
                                            >
                                                <span
                                                    class="text-sm font-weight-medium text-monospace"
                                                >
                                                    {{ $item->ip_address }}
                                                </span>
                                            </div>
